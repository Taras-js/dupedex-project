...
const Tooltip:React.FC<TooltipProps> = (props: TooltipProps) => {
  ...
  const arrowColorGenerator = () => {
    ...
    const arrowColor:string = placementArray.map((el) => (el === placement ? bgColor : 'transparent')).join(' ');
    return arrowColor;
  };

  const tooltipStyle = cls(styles, 'tooltip', className, PlacementTypes[`${placement}`]);
    ...
    <div className={cls(styles, 'tooltip__wrapper')}>
      ...
      <div className={tooltipStyle}>
        <div style={{ fontSize: textSize, backgroundColor: bgColor }} className={tooltipTextStyle}>
          {title}
          <span style={{ borderColor: `${arrowColorGenerator()}` }} className={cls(styles, 'tooltip__arrow', `tooltip__arrow_${placement}`)} />
        </div>
      </div>
    </div>
  );
