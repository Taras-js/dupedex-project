...
router.post("/search", apiLimiter, async (req, res) => {
    ...
    try {
        ...
        let search = await Product.find({brand_name: {$regex: new RegExp('^'+payload+'.*','i')}},
            {brand_name: 1, prod_name: 1, prod_link: 1, price: 1, category: 1, img_link: 1, Benefits: 1,
                Details: 1, Usage: 1, Ingredients: 1, reviews: 1, key_ingredients: 1,}).limit(10).exec()
